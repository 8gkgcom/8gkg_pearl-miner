...
主页：[8gkg.com](https://8gkg.com)。用于 NVIDIA 显卡的 Pearl（PRL / PearlHash）挖矿软件，支持 Windows、Linux 与 HiveOS。默认开发者费用 **1%**，可调整或关闭。

## 文件与启动

| 平台 | 本目录中的发行文件 | 启动方式 |
| --- | --- | --- |
| Windows x64 | `8gkg_pearl-VERSION-win64.zip` | 解压后编辑 `start-pearl.bat` 中的 `WALLET`，保存并双击运行 |
| Linux x64 | `8gkg_pearl-VERSION-linux-x64.tar.gz` | 解压后编辑 `start-pearl.sh` 中的 `WALLET`，执行 `./start-pearl.sh` |
| HiveOS | `8gkg_pearl-VERSION.tar.gz` | 安装自定义矿工，名称填写 `8gkg_pearl` |

Windows 单文件程序为 `8gkg_pearl.exe`。每个发行压缩包旁提供同名 `.sha256` 校验文件。文中的 `VERSION` 表示下载文件中的版本标记，使用时按实际文件名替换。

**正式挖矿前请修改收款钱包。** 无参数启动和随附脚本使用内置测试钱包；不修改时，矿工部分的收益归该默认地址。下方示例统一用 `YOUR_PEARL_ADDRESS` 表示你的 PRL 钱包。
...
```bash
tar -xzf 8gkg_pearl-VERSION-linux-x64.tar.gz
cd 8gkg_pearl
...
本版在矿池确认支持压缩证明后使用 gzip 压缩证明；未确认时使用原始格式。已验证 gzip 编码、完整证明校验和 TLS 授权。矿池网页上的 gzip 兼容性提示是否消失尚未确认，不能只凭本地自检判断矿池页面状态。
...
```text
--pool tls://prl.kryptex.network:8048 --wallet YOUR_PEARL_ADDRESS.WORKER_NAME --password x --devfee 1
```

该写法会以 `YOUR_PEARL_ADDRESS.rig3080` 登录矿池，启动信息中的矿工名也显示 `rig3080`，不再重复追加名称。两种写法任选一种。设备改名后下次启动重新读取名称，配置模板中的 `WORKER_NAME` 保持原样。

设备名优先读取 HiveOS 的 `WORKER_NAME` 环境变量，其次读取 `/hive-config/rig.conf` 中的同名配置，均没有时使用系统主机名。此替换由 HiveOS 启动脚本完成；直接运行 Windows/Linux 主程序时仍填写实际矿工名。
...
| 显卡 | 架构 | 本版测试范围 |
| --- | --- | --- |
| RTX 5060 Ti | SM120 / Blackwell | Windows GPU 源码内核自检；本地 Ubuntu WSL 双卡计算、API、HiveOS 脚本和 gzip 证明测试 |
| RTX 4070 SUPER | SM89 / Ada | Windows GPU 源码内核自检；本地 Ubuntu WSL 双卡计算、API、HiveOS 脚本和 gzip 证明测试 |
| RTX 3080 | SM86 / Ampere | HiveOS / Ubuntu 双卡自动调优、挖矿计算、API 和 HiveOS 脚本测试 |
| NVIDIA CMP 170HX | SM80 / Ampere | HiveOS / Ubuntu 双卡自动调优、挖矿计算、API 和 HiveOS 脚本测试 |

3080 和 CMP 170HX 本版未在 Windows 重测。**RTX 20 系的 SM75 通用内核已编译，尚未实机验收**；其他型号不能仅凭相同架构就视为已测试。
...
开发费使用内置收款地址。`--dev-wallet ADDRESS` 可覆盖该地址；修改矿工的 `--wallet` 不会自动修改开发费收款地址。
...
使用 `--help` 查看可用的内核 ID；日常挖矿保持自动选择即可。
...
程序采用单文件部署，无需安装 .NET 或 CUDA Toolkit；需要支持当前显卡及内置 CUDA 运行时的 NVIDIA 驱动。

Linux 主程序与 CUDA 原生库均在本地 Ubuntu 环境编译，并已在 Ubuntu WSL 和 HiveOS 运行验证。主程序内置运行库，运行时会释放并校验供系统加载的依赖文件；这些文件不保存调优结果。
